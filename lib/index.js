...
class ConnectMongo {
  ...
  save (data, type = '_private') {
    return Promise.all(_.map(data[type], (value, key) => {
      // ensure we're comparing homogenised data
      const valClone = deepClone(value)
      if (_.isEqual(_.at(this.privateCache, key)[0], valClone)) return // skip if not modified
      this.robot.logger.debug(`Saving all ${type} data in ${key} to MongoDB`)
      this.privateCache[key] = valClone
